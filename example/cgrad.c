...
extern void initial_src1(int batch, int channels, int height, int width, double *image_pointer);
extern void initial_src2(int batch, int channels, int height, int width, double *image_pointer);
extern void validate_src_data(int batch, int channels, int height, int width, double *image_pointer);
extern void print_data(int batch, int channels,int height,int width, double *image_pointer);
extern void print_max_pool_plus_add_checksum(int batch, int channels, int height, int width, double *output_pointer);
extern void forward_maxpool_plus_add_fusion_layer(int batch, int in_h, int in_w, int in_c, int stride, int size, int pad, double *src1_pointer, double *src2_pointer, double *dst_pointer);
...
int main(int ac, char *av[]){
	...
	int pooled_size = SRC1_BATCH*SRC1_CHANNELS*pooled_height*pooled_width*sizeof(double);	

	int src1_size = SRC1_BATCH*SRC1_CHANNELS*SRC1_HEIGHT*SRC1_WIDTH*sizeof(double);
        int src2_size = pooled_size;
	int dst_size = pooled_size;
	double *gpu_src1_pointer, *gpu_src2_pointer, *gpu_output_pointer;
	double *src1_pointer, *src2_pointer, *src1_output_pointer, *dst_pointer, *output_pointer;
	
	src1_pointer = (double *) malloc(src1_size);
	src1_output_pointer = (double *) malloc(pooled_size);
	src2_pointer = (double *) malloc(src2_size);
	dst_pointer = (double *) malloc(dst_size);
	output_pointer = (double *) malloc(src2_size);
